...
INSERT INTO customer_profile (id, name, phone, address) VALUES
    (3, 'Lê Văn C', '0901234567', '789 Nguyễn Huệ, Quận 1, TP.HCM');
...
-- DEMO DATASET: replicated product/catalog data and HCM local fragments.
INSERT INTO category (id, name) VALUES
    (3, 'Máy tính bảng'),
    (4, 'Màn hình'),
    (5, 'Bàn phím'),
...
UPDATE inventory SET quantity = 40 WHERE warehouse_id = 3 AND product_id = 1;
UPDATE inventory SET quantity = 90 WHERE warehouse_id = 3 AND product_id = 2;
...
SELECT w.warehouse_id, p.id,
       ((p.id * 9 + w.warehouse_id * 7) % 45) + 15
FROM product_basic p
CROSS JOIN (VALUES (3), (6)) AS w(warehouse_id)
...
SELECT id,
       'Khách hàng HCM ' || id,
       '09012' || LPAD(id::text, 5, '0'),
       'Địa chỉ demo tại TP.HCM ' || id
FROM customer_identity
WHERE main_site_id = 3 AND id <> 3;

INSERT INTO orders (id, customer_id, status, site_id, order_date)
SELECT 3000 + g,
       (ARRAY[3,22,23,24,25,26,27,28,29,30])[((g - 1) % 10) + 1],
       CASE WHEN g <= 25 THEN 'COMPLETED' ELSE 'PENDING' END,
       3,
...
SELECT 3000 + g,
       ((g + 19) % 50) + 1,
       CASE WHEN g % 2 = 0 THEN 6 ELSE 3 END,
       (g % 3) + 1,
       p.price
FROM generate_series(1, 30) AS g
JOIN product_basic p ON p.id = ((g + 19) % 50) + 1;

INSERT INTO order_detail (order_id, product_id, warehouse_id, quantity, price)
SELECT 3000 + g,
       ((g + 24) % 50) + 1,
       CASE WHEN g % 10 = 0 THEN 2 ELSE 4 END,
       1,
       p.price
FROM generate_series(5, 30, 5) AS g
JOIN product_basic p ON p.id = ((g + 24) % 50) + 1;
